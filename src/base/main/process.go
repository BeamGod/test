...
//解决 ： 虽然用编译器可以帮我编译目录下全部文件，但是用命令的时候是不知道哪些文件是需要编译的 。 所以需要 go build main.go process.go  或者直接编译运行该package go run test/src/base/main
...
func testSwitch()  {
	...
	switch i := interface{}(x).(type) {   //switch i := x  报错 不理解（） todo
	case nil:
